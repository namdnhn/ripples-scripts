...
recombination_file_name = "combinedCatOnlyBestWithPVals.txt"
sampleinfo_file_name = "sampleInfo.txt"
relevent_sites_file_name = "allRelevantNodesInfSites.txt"

def process_sample_file(sample_id, recomb_samples, donor_samples, acceptor_samples):   
    output_file = f"RecombFasta/{sample_id}.fa"  
    fasta_file = "RecombFasta/aligned_gisaid_seqs.fa" 
    if os.path.exists(output_file):
        return 

    sequences = []  

    # Đọc file FASTA và trích xuất các mẫu tương ứng  
    for record in SeqIO.parse(fasta_file, "fasta"):
        if record.id in recomb_samples:  
            sequences.append(SeqRecord(record.seq, id=record.id, description=""))
        elif record.id in donor_samples:
            sequences.append(SeqRecord(record.seq, id=record.id, description=""))
        elif record.id in acceptor_samples:
            sequences.append(SeqRecord(record.seq, id=record.id, description=""))

    # Ghi các trình tự vào file FASTA  
    if sequences:  
        SeqIO.write(sequences, output_file, "fasta")  
        print(f"Đã tạo file '{output_file}' chứa trình tự của các mẫu.")  
    else:  
        print(f"Không tìm thấy trình tự nào tương ứng trong file FASTA!")  
...
print("recomb_id", recomb_id)
print("donor_id", donor_id)
print("acceptor_id", acceptor_id)
...
for i in range(len(samplelines)):
    ...
    if int(samplelines[i][0]) == recomb_id:
        ...
        recomb_mutations = list(map(int, samplelines[i][2].split(',')))

    if int(samplelines[i][0]) == donor_id:
        donor_sample = samples[0]
        donor_samples = samples[:10]
        donor_mutations = list(map(int, samplelines[i][2].split(',')))

    if int(samplelines[i][0]) == acceptor_id:
        acceptor_sample = samples[0]
        acceptor_samples = samples[:10]
        acceptor_mutations = list(map(int, samplelines[i][2].split(',')))

print("recomb", recomb_samples)
print("donor", donor_samples)
print("acceptor", acceptor_samples)



process_sample_file(index_to_check, recomb_samples, donor_samples, acceptor_samples)
...
# print("acceptor_mutation", acceptor_mutations)
...
dna = open("RecombFasta/%d.fa" % index_to_check)
...
# print("recomb_reads", trecomb_reads)
# print("donor_reads", tdonor_reads)
# print("acceptor_reads", tacceptor_reads)
...
#     # print("mutations", mutations)
#     # print("recomb_reads", recomb_reads)
#     # print("donor_reads", donor_reads)
#     # print("acceptor_reads", acceptor_reads)
...
for i in range(len(mutations)):
    ...
    in_good_region = 200 < int(mutations[i]) and int(mutations[i]) < 29800

    if(int(mutations[i]) <= break1_1):
    ...
    if(break1_1 < int(mutations[i]) and int(mutations[i]) < break1_2):
    ...
    if(break1_2 <= int(mutations[i]) and int(mutations[i]) <= break2_1):
    ...
    if(break2_1 < int(mutations[i]) and int(mutations[i]) < break2_2):
...
# print("recomb_mutations", recomb_mutations)
# print("donor_mutations", donor_mutations)
# print("acceptor_mutations", acceptor_mutations)

# print(recomb_mutations[0])
# print(type(recomb_mutations[0]))
...
print('most clumps in', most_clumps_in)
